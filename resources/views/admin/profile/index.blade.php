<x-layout>
   ...
   <main>
      <div class="mx-auto max-w-screen-2xl p-4 md:p-6 2xl:p-10">
         <h4 class="text-lg font-bold text-black dark:text-white mb-4">Profil Saya</h4>
         <div class="rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark">
            ...
            <div class="p-6.5 grid grid-cols-1 gap-y-5 sm:grid-cols-2 sm:gap-x-8">
               ...
               <div class='mb-4'>
                  ...
                  <p class="text-base font-medium text-black dark:text-white">{{ $profil['nama'] }}</p>
               </div>
               <!-- Email -->
               <div class='mb-4'>
                  <p class="text-sm text-slate-500 mb-1">Email</p>
                  <p class="text-base font-medium text-black dark:text-white">{{ $profil['email'] }}</p>
               </div>
               <!-- No HP -->
               <div class='mb-4'>
                  <p class="text-sm text-slate-500 mb-1">No HP</p>
                  <p class="text-base font-medium text-black dark:text-white">{{ $profil['no_hp'] }}</p>
               </div>
               <!-- Role -->
               <div class='mb-4'>
                  <p class="text-sm text-slate-500 mb-1">Role</p>
                  <p class="text-base font-medium text-black dark:text-white">{{ $profil['role'] }}</p>
               </div>
               <!-- Status -->
               <div class='mb-4'>
                  <p class="text-sm text-slate-500 mb-1">Status</p>
                  <span class="inline-flex rounded-full px-3 py-1 text-sm font-medium
                     {{ $profil['status'] == 'aktif' ? 'bg-success bg-opacity-10 text-success' : 'bg-danger bg-opacity-10 text-danger' }}">
                     {{ ucfirst($profil['status']) }}
                  </span>
               </div>
               <!-- Tombol Edit Profil -->
               <div class="sm:col-span-2 flex justify-end mt-6">
                  <a href="{{ route('admin.profile.edit') }}" class="px-4 py-2 text-white bg-primary rounded-md hover:bg-primary-dark">
                     Edit Profil
                  </a>
               </div>
            </div>
