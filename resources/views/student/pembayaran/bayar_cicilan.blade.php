            ...
            <div class="rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark">
                ...
                <div class="p-6.5 grid grid-cols-1 gap-y-5 sm:grid-cols-2 sm:gap-x-8">
                    <div>
                        ...
                        <p class="text-base font-medium text-black dark:text-white mb-2">{{ $cicilan->riwayat->siswa->nama_lengkap ?? "-" }}</p>
                    </div>
                    <div>
                        <p class="text-sm text-slate-500 mb-1">Jenis Pembayaran</p>
                        <p class="text-base font-medium text-black dark:text-white mb-2">{{ $cicilan->riwayat->tagihan->judul }}</p>
                    </div>
                    <div>
                        <p class="text-sm text-slate-500 mb-1">Keterangan</p>
                        <p class="text-base font-medium text-black dark:text-white mb-2">{{ $cicilan->riwayat->tagihan->deskripsi}}</p>
                    </div>
                    <div>
                        <p class="text-sm text-slate-500 mb-1">Nominal Bayar</p>
                        <p class="text-base font-medium text-black dark:text-white mb-2">Rp
                            {{ number_format($cicilan->nominal, 0, ',', '.') }}</p>
                    </div>
                    <div>
                        <p class="text-sm text-slate-500 mb-1">Tanggal Tempo</p>
                        <p class="text-base font-medium text-black dark:text-white mb-2">{{ $cicilan->tanggal_tempo}}
                        </p>
                    </div>
                    <div>
                        <p class="text-sm text-slate-500 mb-1">Tanggal Bayar</p>
                        <p class="text-base font-medium text-black dark:text-white mb-2">{{ $cicilan->tanggal_bayar ?? "-"}}
                        </p>
                    </div>
                     <div>
                        <p class="text-sm text-slate-500 mb-1">Metode</p>
                        <p class="text-base font-medium text-black dark:text-white mb-2">{{ $cicilan->metode ?? "-"}}</p>
                    </div>
                    <div>
                        ...
                        <p
                            ...
                            {{
                                $cicilan->status == 'Lunas' || $cicilan->status == 'lunas'
                                    ? 'bg-success bg-opacity-10 text-success'
                                    : (
                                        $cicilan->status == 'Pending' || $cicilan->status == 'pending'
                                            ? 'bg-warning bg-opacity-10 text-warning'
                                            : (
                                                $cicilan->status == 'Cicilan' || $cicilan->status == 'cicilan'
                ...
                <div class="mt-6 px-6.5 pb-6.5 flex justify-end mb-4">
                    @if(strtolower($cicilan->status) !== 'lunas')
