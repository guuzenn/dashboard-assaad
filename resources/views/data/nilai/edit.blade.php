<x-layout>
  ...
  <main>
      <div class="mx-auto max-w-screen-2xl p-4 md:p-6 2xl:p-10">
         <h4 class="text-lg font-bold text-black dark:text-white mb-4">Edit Nilai</h4>

         <div class="grid grid-cols-1 gap-9 sm:grid-cols-2">
            <div class="flex flex-col gap-9">
               <div class="rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark">
                  <div class="border-b border-stroke px-6.5 py-4 dark:border-strokedark">
                     <h3 class="font-medium text-black dark:text-white">Form Edit Nilai</h3>
                  </div>
                    ...
                    <form action="{{ route('data.nilai.update', $nilai->id) }}" method="POST">
                     ...
                     <div>
                        ...
                        >
                           ...
                           @foreach ($kelas as $k)
                              <option value="{{ $k->id }}" @selected($nilai->kelas_id == $k->id)>
                                 {{ $k->nama }}
                              </option>
                           @endforeach
                        </select>
                     ...
                     <!-- Semester -->
                     <div>
                        <label class="mb-3 block text-sm font-medium text-black dark:text-white">Semester</label>
                        <select
                           name="semester"
                           class="w-full rounded-lg border-[1.5px] border-stroke bg-transparent px-5 py-3 font-normal text-black outline-none transition focus:border-primary dark:border-form-strokedark dark:bg-form-input dark:text-white"
                        >
                        ...
                        >
                           <option value="">Pilih Semester</option>
                            @foreach ($mapel as $item)
                                <option value="{{ $item->id }}"{{ old('mata_pelajaran_id', $nilai->mata_pelajaran_id) == $item->id ? 'selected' : '' }} >{{ $item->nama }}</option>
                            @endforeach
                        </select>
