<x-layout>
  ...
  <main>
      <div class="mx-auto max-w-screen-2xl p-4 md:p-6 2xl:p-10">
         <h4 class="text-lg font-bold text-black dark:text-white mb-4">Edit Kelas</h4>
         <div class="grid grid-cols-1 gap-9 sm:grid-cols-2">
            <div class="flex flex-col gap-9">
               <div class="rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark">
                  <div class="border-b border-stroke px-6.5 py-4 dark:border-strokedark">
                     <h3 class="font-medium text-black dark:text-white">Form Edit Kelas</h3>
                  </div>
                  <div class="flex flex-col gap-5.5 p-6.5">
                    <form action="{{ route('data.pivot_mapel_kelas.update', $pivot->id) }}" method="POST">
                        @csrf
                        @method('PUT')
                     <!-- Kelas -->
                     <div>
                        ...
                        >
                           ...
                           @foreach ($kelas as $item)
                              <option value="{{ $item->id }}" {{ old('kelas_id', $pivot->kelas_id) == $item->id ? 'selected' : '' }}>
                                 {{ $item->nama }}
                              </option>
                           @endforeach
                        </select>
                      ...
                      <div>
                        ...
                        >
                           ...
                           @foreach ($mata_pelajaran as $item)
                              <option value="{{ $item->id }}" {{ old('mata_pelajaran_id', $pivot->mata_pelajaran_id) == $item->id ? 'selected' : '' }}>
                                 {{ $item->nama }}
                              </option>
                           @endforeach
                        </select>
