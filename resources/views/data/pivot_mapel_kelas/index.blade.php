<x-layout>
   ...
   <main>
      <div class="mx-auto max-w-screen-2xl p-4 md:p-6 2xl:p-10">
         <h4 class="text-lg font-bold text-black dark:text-white mb-4">Data Pembagian Mata Pelajaran per Kelas</h4>
         <!-- Card Statistik -->
         <div class="grid grid-cols-1 gap-4 md:grid-cols-2 md:gap-6 xl:grid-cols-4 2xl:gap-7.5">
            <!-- Card Item Start -->
            <div class="rounded-sm border border-stroke bg-white px-7.5 py-6 shadow-default dark:border-strokedark dark:bg-boxdark">
               <div class="flex h-11.5 w-11.5 items-center justify-center rounded-full bg-meta-2 dark:bg-meta-4">
                  <svg class="fill-primary dark:fill-white" width="20" height="22" viewBox="0 0 24 24" fill="none"
                     xmlns="http://www.w3.org/2000/svg">
               ...
               <div class="mt-4 flex items-end justify-between">
                  <div>
                     <h4 class="text-title-md font-bold text-black dark:text-white">
                        {{ $pivot->pluck('kelas_id')->unique()->count() }}
                     </h4>
         ...
         <div class="rounded-sm border border-stroke bg-white px-5 pb-2.5 pt-6 shadow-default dark:border-strokedark dark:bg-boxdark sm:px-7.5 xl:pb-1 mt-6">
            <div class="flex items-center justify-between mb-4">
               <h4 class="text-lg font-bold text-black dark:text-white">List Pembagian Mata Pelajaran per Kelas</h4>
               <button class="px-4 py-2 text-white bg-primary rounded-md hover:bg-primary-dark"
                  @click="window.location.href='{{ route('data.pivot_mapel_kelas.create') }}'">
               Atur Mata Pelajaran
               </button>
            </div>
               ...
               <table class="w-full table-auto">
                  <thead>
                     <tr class="bg-gray-2 text-left dark:bg-meta-4">
                        ...
                        <th class="min-w-[150px] px-4 py-4 font-medium text-black dark:text-white">Mata Pelajaran</th>
                        <th class="px-4 py-4 font-medium text-black dark:text-white">Aksi</th>
                     </tr>
                  </thead>
                  <tbody>
                     @foreach ($pivot as $index => $item)
                     <tr>
                        ...
                        <td class="border-b border-[#eee] px-4 py-5 dark:border-strokedark">{{ $item->mataPelajaran->nama }}</td>

                        <td class="border-b border-[#eee] px-4 py-5 dark:border-strokedark">
                            <form action="{{ route('data.pivot_mapel_kelas.destroy', $item->id) }}" method="POST">
                                @csrf
                                @method('DELETE')
                           <div class="flex items-center space-x-3.5">
                              <!-- Button Show -->
                              <a href="{{ route('data.pivot_mapel_kelas.show', $item->id) }}" class="hover:text-primary">
                                 <svg
                                    class="fill-current"
                                    width="18"
                                    height="18"
                              ...
                              <!-- Button Edit -->
                              <a href="{{ route('data.pivot_mapel_kelas.edit', $item->id) }}" class="hover:text-primary">
                                 <svg
                                    class="fill-current"
                                    width="18"
                                    height="18"
